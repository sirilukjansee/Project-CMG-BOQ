...
@section('content-data')
                ...
                <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
                    <div class="text-center">
                        ...
                        <a href="javascript:;" data-tw-toggle="modal" data-tw-target="#large-modal-size-import" class="btn btn-success mr-1 mb-2 text-white">
                            <i data-lucide="database" class="w-4 h-4 mr-2"></i> Import Vender
                        </a>
                        <a href="{{ url('/export-vender')}}" class="btn btn-pending mr-1 mb-2 text-white">
                            <i data-lucide="file-text" class="w-4 h-4 mr-2"></i> Export Vender
                        </a>
                        <a href="javascript:;" id="btn_add" data-tw-toggle="modal" data-tw-target="#large-modal-size-preview_add" class="btn btn-primary mr-1 mb-2">
                            <i data-lucide="plus" class="w-4 h-4 mr-2"></i> Add Vender
                        </a>
            ...
            <div class="intro-y box p-5 mt-5">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <!-- BEGIN: Large Modal Toggle -->
                                    <button class="btn btn-secondary mr-2 mb-2" onclick="edit_modal({{$vd->id}})" data-tw-toggle="modal"
                                        data-tw-target="#large-modal-size-preview_edit"> <i data-lucide="edit-2" class="w-4 h-4 mr-2"></i> Edit</button>

                                    @if ($vd->is_active == "1")
                                        <span class="form-switch">
                                            <input id="" class="form-check-input status" type="checkbox" value="{{$vd->id}}" checked>
                                            <label class="form-check-label message_status{{$vd->id}}" for="checkbox-switch-7">ON</label>
                                        </span>
                                    @else
                                        <span class="form-switch">
                                            <input id="" class="form-check-input status" type="checkbox" value="{{$vd->id}}">
                                            <label class="form-check-label message_status{{$vd->id}}" for="checkbox-switch-7">OFF</label>
                                        </span>
                                    @endif
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h2 class="font-medium text-base mr-auto">Add Vender</h2>
                            ...
                            <form action="{{ url('/masterVender/add') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                                    <div class="col-span-12 sm:col-span-12 input-form mt-3">
                                        <input type="text" class="form-control mb-2 chk_name" name="name" placeholder="Please add a Name..." required>
                ...
                <div id="large-modal-size-preview_edit" class="modal" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h2 class="font-medium text-base mr-auto">Edit Vender</h2>
                            </div> <!-- END: Modal Header -->
                            <!-- BEGIN: Modal Body -->
            ...
            <div id="large-modal-size-import" class="modal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h2 class="font-medium text-base mr-auto">Import Vender</h2>
                        </div> <!-- END: Modal Header -->
                        <!-- BEGIN: Modal Body -->
                        <form action="{{url('/import-vender')}}" method="post" enctype="multipart/form-data">
                            @csrf
                            <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
...
<script type="text/javascript">
        ...
        if($(this).is(':checked') )
            {
                ...
                jQuery.ajax({
                type:   "GET",
                url:    "{!! url('/masterVender/changeStatus/"+id+"') !!}",
                datatype:   "JSON",
                async:  false,
                    success: function() {}
                });
    ...
    function edit_modal(id){
     ...
     $('.chk_name').on('keyup', function() {
            ...
            jQuery.ajax({
                type:   "GET",
                url:    "{!! url('masterVender/chk/"+datakey+"') !!}",
                datatype:   "JSON",
                async:  false,
                success: function(data) {
