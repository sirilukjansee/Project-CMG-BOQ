...
@section('content-data')
            <div class="intro-y flex sm:flex-row items-center mt-3">
                <h2 class="text-lg font-medium mr-auto mt-5 mb-5">
                    <b>Create Auctual {{ @$project_id->brand_master->brand_name }} at {{ @$project_id->location_master->location_name }}</b>
                </h2>
                <a href="{{ url('auctual/export', $project_id->id) }}"
            ...
            <!-- BEGIN: Validation Form -->
                <div class="group_wrapper">
                    <div class="intro-y input-form box p-5 mt-3">
                        <form action="{{ route('addauctual') }}" method="post" id="form1" name="form1" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="grid grid-cols-12 gap-2">
                                @foreach ($data_categories as $key => $cat)
                                    ...
                                    @foreach ( $aut as $at )
                                        @if ( $at->boq_id == $cat->id )
                                            <input type="text" name="remark[]" step=".01" class="w-full" value="{{ $at->remark }}" style="text-align: left;" >
                                        @endif
                                    @endforeach
                                    @if ( count($aut) == 0)
                                    ...
                                    <div class="col-span-12 xl:col-span-3 input-form mt-3">
                                        @foreach ( $aut as $at1 )
                                            @if ( $at1->boq_id == $cat->id )
                                                <input type="number" name="total[]" step=".01" class="w-full" value="{{ $at1->total }}" placeholder="0000.00" style="text-align: right;" >
                                            @endif
                                        @endforeach
                                        @if ( count($aut) == 0)
                            ...
                            <div class="grid grid-cols-12 gap-2">
                                <div class="col-span-12 xl:col-span-9 input-form mt-3">
                                    <input type="text" class="text-right w-full" value="Total Contruction" readonly style="background-color: rgb(255, 255, 255);" >
                                </div>
                                <div class="col-span-12 xl:col-span-3 input-form mt-3">
                                    <input type="text" name="total_sqm"  class="w-full total" value="{{number_format($tot1,2)}}" placeholder="0000.00" style="text-align: right; background-color: rgb(255, 255, 255);" readonly>
                                </div>
                            </div>
                            <div class="grid grid-cols-12 gap-2">
                                <div class="col-span-12 xl:col-span-9 input-form mt-3">
                                    <input type="text" class="text-right w-full" value="Total Cost/Sq.m" readonly style="background-color: rgb(255, 255, 255);" >
                                </div>
                                <div class="col-span-12 xl:col-span-3 input-form mt-3">
                                    <input type="text" name="total_sqm"  class="w-full total" value="{{ number_format($tot1 /$project_id->area, 2) }}" placeholder="0000.00" style="text-align: right; background-color: rgb(255, 255, 255);" readonly>
                                </div>
                            </div>
                            <div class="grid grid-cols-12 gap-2">
                                <div class="col-span-12 xl:col-span-9 input-form mt-3">
